...
If you are deploying with compute nodes on different networks to the controller, some additional changes are need so that the metrics sent over AMQ can reach the SAF.
The following needs to be added under the `ExtraConfig` section for each compute node.
...
In the above example, `internal_apiN` is the network that the compute node is connected to.
For more information on this configuration, see: https://access.redhat.com/documentation/en-us/red_hat_openstack_platform/13/html-single/spine_leaf_networking/
