...
If you are deploying compute nodes on networks different from the controllers, some additional changes are required to allow data transported via AMQ INterconnect to reach the SAF server instance.

The following configuration exambple needs to be added under the `ExtraConfig` section for each compute node.
