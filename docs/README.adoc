...
If nodes are on a separate network fronm the default `InternalApi` network used by the controllers, which is typical in a Spine and Leaf or a Distributed Compute Node topology, then a few changes are necessary to allow data transported via AMQ Interconnect to reach the SAF server instance.
...
image::OpenStack_SAF_Overview_37_0919_topology.png[SAF Multi-Cloud Architecture Diagram]
